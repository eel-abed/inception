...
### SSH Connection for Evaluation (Clipboard Sharing)
VirtualBox natively restricts clipboard sharing (copy/pasting) without installing heavy Guest Additions. To ensure a smooth peer evaluation, access the VM from the host machine's terminal via SSH:
1. Configure Port Forwarding in VirtualBox: map Host Port `4242` to Guest Port `22`.
2. Open your school workstation terminal and log in with:
```bash
ssh -p 4242 eel-abed@127.0.0.1
```
This allows terminal scrollback, split panes, and instant copy/pasting.

### Database Verification (Evaluation Requirement)
To prove during the evaluation that the database connects properly and is not empty, use the following commands from your host terminal:

1. Connect to the MariaDB container interactively:
```bash
docker exec -it mariadb mariadb -u root -p
```
2. When prompted, enter the root password.
3. Once in the MariaDB prompt, verify the WordPress database exists:
```sql
SHOW DATABASES;
USE wordpress_db;
```
4. Prove the database is fully populated by WordPress:
```sql
SHOW TABLES;
5. Select user data to confirm content:
SELECT * FROM wp_users;
```
*(You will see all the `wp_` tables generated by the WordPress installation).*
